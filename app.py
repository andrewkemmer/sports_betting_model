...
sport_key = st.sidebar.selectbox("Sport",
...
def extract_game_lines(odds_df: pd.DataFrame) -> pd.DataFrame:
    ...
    spreads = odds_df[odds_df["market"]=="spreads"].groupby("game_id",as_index=False)["line"].mean().rename(columns={"line":"spread_close"})
    totals = odds_df[odds_df["market"]=="totals"].groupby("game_id",as_index=False)["line"].mean().rename(columns={"line":"total_close"})
    teams = odds_df.groupby("game_id",as_index=False).agg(home_team=("home_team","first"),away_team=("away_team","first"))
    return teams.merge(spreads,on="game_id",how="left").merge(totals,on="game_id",how="left")

# Scores helpers (completed games only)
def fetch_scores_with_odds(api_key,sport="basketball_nba",days_back=30)->pd.DataFrame:
    # Use a single request with daysFrom to avoid repeated calls and rate-limit pressure
    rows=[]
    ...
    for game in r.json():
        ...
        rows.append({"game_id":game.get("id"),"date":date,"home_team":home,"away_team":away,
                     "home_score":home_score,"away_score":away_score,"spread_close":None,"total_close":None})
...
def retrain_and_log(df: pd.DataFrame, sport="basketball_nba", impute="none"):
    ...
    # Require at least one non-null feature per row; drop rows with both features missing
    ...
    if impute == "median":
        ...
        # drop rows in train/test that have all features missing (should be uncommon after earlier dropna)
    ...
    # Predict and evaluate with safe ordering and indexed y_test
...
def predict_scores_from_lines(df: pd.DataFrame, model, impute="none"):
    ...
    if mask_spread.any():
        ...
        if impute == "median":
            ...
            # drop rows where both features are missing
        ...
        X_valid = X_valid[feature_cols]  # enforce column order

        # Debug: see input stats
        ...
        # Heuristic predicted margin (optional): keep but treat as heuristic
        df.loc[X_valid.index, "predicted_margin"] = df.loc[X_valid.index, "spread_close"] * (0.5 + df.loc[X_valid.index, "model_prob_home_win"])

        # Debug check for constant probabilities
    ...
    # compute totals and scores only where total is present and predicted_margin exists
...
# Model loader with caching to avoid reloading repeatedly
@st.cache_resource
...
if btn_fetch and api_key:
    ...
    if not odds_df.empty:
        ...
        if model is not None:
            preds = predict_scores_from_lines(lines_df, model, impute=("median" if impute_option=="median" else "none"))
...
if btn_retrain and api_key:
    with st.spinner("Fetching historical scores and retraining..."):
        ...
        if not scores_df.empty:
            try:
                model, X_test, y_test, y_prob, metrics = retrain_and_log(scores_df, sport_key, impute=("median" if impute_option=="median" else "none"))
