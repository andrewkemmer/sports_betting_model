...
# -------------------------
# Model training (classification: home win probability)
# -------------------------
...
def predict_scores_from_lines(df: pd.DataFrame, model):
    ...
    # Win probabilities and margin (margin uses spread + confidence)
    ...
    if mask_spread.any():
        ...
        # Margin heuristic: don't collapse to 0 when prob ~ 0.5
        df.loc[mask_spread, "predicted_margin"] = df.loc[mask_spread, "spread_close"] * (0.5 + probs)

    # Scores only if totals exist
